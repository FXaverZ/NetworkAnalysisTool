...
%------------------------------------------------------------------------------------
% Eine Netzanalyse mit generischen Lasten durchführen:
%------------------------------------------------------------------------------------
tic;
disp('----');
disp('Führe Netzanalyse durch...');
...
Grid.Load.Loads.update_power(1);
...
Grid.Properties.Voltage_no_load =  vertcat(Grid.P_Q_Node.Points.Voltage);

test_p = 1000;

% Netz mit generischen Lasten untersuche:
Grid.Properties.dV_dPQ = zeros(6,numel(Grid.P_Q_Node.ids)*3);
for i=1:numel(Grid.P_Q_Node.ids)
	% für alle Phasen:
	for j=1:6
		p_q = zeros(1,6);
		p_q(j) = test_p/1e6;
		Grid.Load.Loads(i).P_Q_t = p_q;
		...
		% Spannungswerte aktualisieren:
		Grid.P_Q_Node.Points(i).update_voltage_node_LF_USYM;
		
		% Spannungswert auslesen:
		dV_dQ = Grid.P_Q_Node.Points(i).Voltage - Grid.Properties.Voltage_no_load(i);
		Grid.Properties.dV_dPQ(j,(i-1)*3+1:i*3)= dV_dQ/test_p;
	end
	
	% die genwonnenen Informationen auch in den Netzknoten speichern:
	Grid.P_Q_Node.Points(i).dV_dPdQ = Grid.Properties.dV_dPQ(:,(i-1)*3+1:i*3);
	
	% Statusinfo zum Gesamtfortschritt an User:
	t = toc;
	progress = i/numel(Grid.P_Q_Node.ids);
	time_elapsed = t/progress - t;
	disp(['Untersuchung von Netzknoten Nr. ',num2str(i),' von ',...
		num2str(numel(Grid.P_Q_Node.ids)),' abgeschlossen. Laufzeit: ',...
		sec2str(t),...
		'. Verbleibende Zeit: ',...
