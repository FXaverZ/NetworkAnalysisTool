...
%    aufbereiteten Daten bzw. die Funktionen für die Aufbereitung der Daten für die
%    Darstellung im Daten Explorer enthält. So sind innerhalb dieser Struktur neben
%    den Funktionenhandles auf Bearbeitungsfunktionen, Verweis auf die entsprechenden
%    Rohdaten auch die Beschreibung, Einheit, Legendeneinträge und
%    Achsenbeschriftungen vorhanden, auf die der 'Data_Explorer' zurückgreift.
...
if ~isempty(Result.Households.Data_Min) && ~isempty(Result.Households.Data_Max)
	% Anzeigbare Kurven definieren:
	crv_cnt = crv_cnt + 1;
	crv_nam = ['Curve_',num2str(crv_cnt)];
	dspl.(crv_nam).Title = ['Gesamtwirkleistungsaufnahme Haushalte (',...
...
if ~isempty(Result.Households.Data_05P_Quantil) &&...
		~isempty(Result.Households.Data_95P_Quantil)
	% Anzeigbare Kurven definieren:
	crv_cnt = crv_cnt + 1;
	crv_nam = ['Curve_',num2str(crv_cnt)];
	dspl.(crv_nam).Title = ['Gesamtwirkleistungsaufnahme Haushalte (',...
...
% end
% 
% if ~isempty(Result.Wind.Data_Min) &&  ~isempty(Result.Wind.Data_Max)
% 	crv_cnt = crv_cnt + 1;
% 	crv_nam = ['Curve_',num2str(crv_cnt)];
% 	dspl.(crv_nam).Title = ['Gesamteinspeisung Windkraft-Anlagen (',...
% 		'Minimal und Maximalwerte',')'];
