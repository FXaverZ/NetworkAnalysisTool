...
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Description = ...
	'Base Scenario - No Infeed, 0% Elektormobility, Normal Load';
...
% Bezeichnung des Szenarios:
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Description = ...
	'Scenario 1 - High Infeed, 30% Elektormobility, Low Load (0-25)%';
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Filename = '02_Scenario_1';
% Erzeugungsanlagen verteilen (gemäß Parametern):
...
% Bezeichnung des Szenarios:
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Description = ...
	'Scenario 2 - Medium Infeed, 75% Elektormobility, High Load (75-100)%';
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Filename = '03_Scenario_2';
% Erzeugungsanlagen verteilen (gemäß Parametern):
...
% Bezeichnung des Szenarios:
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Description = ...
	'Scenario 3 - High Infeed, 25% Elektormobility, Low Load (0-25)%';
Simulation.Scenarios.(['Sc_',num2str(scen_count)]).Filename = '04_Scenario_3';
% Erzeugungsanlagen verteilen (gemäß Parametern):
Solar.Number = [75, 0];         % Anteil der Anlagen an Gesamtanzahl an Anschlussknoten [% Fix, % Tracker]
% Solar.Power_tot = 20;         % gesamte Leistung aller Anlagen [kWp]
Solar.Power_sgl = 10;            % mittlere Leistung der Anlagen [kWp]
