function Scenario_Output = create_result_structure_with_timeperiod(handles,data,grid_data,timeperiod)
...
Timepoints = numel(timeperiod);

if handles.NVIEW_Control.Simulation_Options.Voltage_Analysis == 1
    if Timepoints ~= handles.NVIEW_Control.Simulation_Options.Timepoints_per_dataset
        data = recalculate_summary(handles,data,timeperiod,'voltage');
    end
end
if handles.NVIEW_Control.Simulation_Options.Overcurrent_Analysis == 1
    if Timepoints ~= handles.NVIEW_Control.Simulation_Options.Timepoints_per_dataset
        data = recalculate_summary(handles,data,timeperiod,'branch');
    end
end
if handles.NVIEW_Control.Simulation_Options.Loss_Analysis == 1
    if Timepoints ~= handles.NVIEW_Control.Simulation_Options.Timepoints_per_dataset
        data = recalculate_summary(handles,data,timeperiod,'losses');
    end
end
if Timepoints ~= handles.NVIEW_Control.Simulation_Options.Timepoints_per_dataset
    data = recalculate_summary(handles,data,timeperiod,'load/infeed');
end
