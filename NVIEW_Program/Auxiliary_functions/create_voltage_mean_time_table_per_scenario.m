function Table = create_voltage_mean_time_table_per_scenario (d,Data_List)
    ...
    for i = 1 : numel(Data_List)
        ...
        for ds = 1 : d.Control.Simulation_Options.Number_of_datasets
            ...
            voltage_timeline(id_,i) = ...
				squeeze(nanmean(squeeze(nanmean(squeeze(d.(Data_List{i}).bus_voltages(s,ds,:,:,:)),2)),2));
        end
    end
    Table.(['Scen_',int2str(s)]).Values = voltage_timeline;
    Table.(['Scen_',int2str(s)]).Description =  ['Voltage mean values timelines for scenario ',scenario_description{s}];
