function histogram_electric_losses_overall(handles,d)
...
Table.Values = [];
for i = 1 : numel(cg)    
Table.Values(:,i) = d.(cg{i}).electric_losses_at_dataset(:)/1000;
end, clear i   
Table.Description = 'Electric losses histogram';


% Create histogram
nBins = 50; % 50 bin histogram!
% Bin edges from 0 to max_edge
binEdges = linspace(min(min(Table.Values)),max(max(Table.Values)),nBins+1);
% Lower edge
aj = binEdges(1:end-1);
% Higher edge
bj = binEdges(2:end);
...
HistXLabel = 'Electric losses in kWh/h';
...
hold off;
xlim([min(min(Table.Values)),max(max(Table.Values))])
