...
for H = 1 : numel(Data_List)
    if ~strcmp(Data_List{H,1},'Input_Data')
        ...
        NVIEW_Results.(Data_List{H}).bus_voltages = NVIEW_Results.(Data_List{H}).bus_voltages(Selected_Scenarios,:,SelectedTimepoints,:,:);
        NVIEW_Results.(Data_List{H}).branch_values = NVIEW_Results.(Data_List{H}).branch_values(Selected_Scenarios,:,SelectedTimepoints,:,:);
        NVIEW_Results.(Data_List{H}).loss_statistics = NVIEW_Results.(Data_List{H}).loss_statistics(Selected_Scenarios,:,SelectedTimepoints,:);
    end
end
