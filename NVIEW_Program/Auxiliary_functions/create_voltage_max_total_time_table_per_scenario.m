function Table = create_voltage_max_total_time_table_per_scenario (d,Data_List)
    ...
    for i = 1 : numel(Data_List)  
         for t = 1 : d.Control.Simulation_Options.Timepoints_per_dataset
            sorted_values.(['Scen_',int2str(s)])(t,i) = ...
                nanmax(nanmax(squeeze(squeeze(nanmax(squeeze(d.(Data_List{i}).bus_voltages(s,:,t,:,:)))))));
         end
    end
        
    Table.(['Scen_',int2str(s)]).Values = sorted_values.(['Scen_',int2str(s)]);
    Table.(['Scen_',int2str(s)]).Description =  ['Total Voltage maximum values for scenario ',scenario_description{s}];
    Table.(['Scen_',int2str(s)]).RowName =  'Maximum voltage level at all nodes [-]';
