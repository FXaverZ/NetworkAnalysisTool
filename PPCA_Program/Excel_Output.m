            ...
            for S = 1 : numel(ext_obj.Scenarios)
                ...
                if S == 1
					...
					for hS = 1 : numel(ext_obj.Scenarios)
						 h.Column_Grid_Scenario_locations(hS,:) = (1:numel(ext_obj.Grid_Variants)) + numel(ext_obj.Grid_Variants)*(hS-1);
					end
                    ...
                    for hG = 1 : numel(ext_obj.Grid_Variants)
                        idg = h.Column_Grid_Scenario_locations(1,hG);
                        h.Column_IDs_sheet2table1{2,idg} = ext_obj.Grid_Variants{hG};
                        for hS = 1 : numel(ext_obj.Scenarios)
                            ids = h.Column_Grid_Scenario_locations(hS,hG);
                            h.Column_IDs_sheet2table1{3,ids} = ext_obj.Scenarios{hS};
                        end
                    end
