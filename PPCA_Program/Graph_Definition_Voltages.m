classdef Graph_Definition_Voltages < handle    
    ...
    methods
        ...
        function obj = Graph_Definition_Voltages()
            % Clear variables
        end
        ...
        function display_datasets(obj,ext_obj,input1,input2)
            ...
            graph_.voltage_violations = flipdim(input1,1);   
            graph_.nodes_violated = flipdim(input2,1);   
        ...
        function display_node_voltage(obj,ext_obj,input)
            ...
            graph_.title = ['Voltages for grid ', ext_obj.Grid_Variants{obj.G},...
                ', scenario ', ext_obj.Scenarios{obj.S}, ', dataset ', int2str(obj.D), ' and node ',obj.N   ];
        ...
        function display_node_variations_datasets(obj,ext_obj,input)
            %----------------------------------------------------
            ...
            if numel(obj.D) ~= 1
                graph_.title = ['Voltages for grid ', ext_obj.Grid_Variants{obj.G},...
                    ', scenario ', ext_obj.Scenarios{obj.S}, ', all datasets'];
            else
                graph_.title = ['Voltages for grid ', ext_obj.Grid_Variants{obj.G},...
                    ', scenario ', ext_obj.Scenarios{obj.S}, ', dataset ', int2str(obj.D)];
            end
        ...
        function display_node_variations_scenarios(obj,ext_obj,input)
        ...
        function display_node_variations_grids(obj,ext_obj,input)
            ...
            for G = 1 : numel(ext_obj.Grid_Variants)
                myplot1 = plot_line_graphs(1);
                graph_.title{G} = ['Voltages for grid ', ext_obj.Grid_Variants{G},...
                    ', all scenarios and all datasets'];
        ...
        function histogram_comparisons_grids_at_scenario(obj,ext_obj,input)
            ...
            if ext_obj.x_axis_value == 2
                graph_.x.label = 'Voltage violations in % of observed timepoints for the observed scenario load profiles';
            elseif ext_obj.x_axis_value == 1
               graph_.x.label = 'Number of voltage violations during observations for the observed scenario load profiles'; 
            end
            graph_.title = ['Histogram of voltage violations for scenario ', ext_obj.Scenarios{obj.S}];
        ...
        function histogram_comparisons_scenarios_at_grid(obj,ext_obj,input)
            ...
            graph_.title = ['Histogram of voltage violations for grid ', ext_obj.Grid_Variants{obj.G}];
