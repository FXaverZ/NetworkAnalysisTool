...
try
	start_gui_Input = find(strcmp(varargin, 'NAT_main'));
...
if dontOpen
	...
	disp('übergeben werden dessen Name ''NAT_main'' und Wert der Handle')
	disp('auf das GUI von Acces_Tool.m ist! z.B.:');
	disp('   x = NAT_main();');
	disp('   Data_Explorer(''NAT_main'', handles.accesstool_main_window);');
	disp('---------------------------------------------------------------');
...
% Daten berechnen:
data = disp.Data_fun(Result.(args{1}),args{2:end});
% Legendeneinträge:
