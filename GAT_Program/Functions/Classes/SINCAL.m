classdef SINCAL < handle
	...
	methods
		...
		function entry = get_entry_table(obj,table_name,name_identifier,el_id,property_name)
			...
			% get the entriey along with their IDs
			entry = obj.Tables.(table_name)(idx_elemt,idx_prope);
			el_id_entry = cell2mat(obj.Tables.(table_name)(idx_elemt,idx_el_id));
			% Sort the entries back to the given order in the function call
			% (due to the index operation the original order is lost!)
			entry_sort = entry;
			for i=1:numel(el_id)
				entry_sort(i) = entry(el_id(i) == el_id_entry);
			end
			entry = entry_sort;
			
			% if strings are present, deblank them, otherwise transform
			% cells to matrices
			if ~isempty(entry) && ischar(entry{1,1})
				entry = deblank(entry);
