...
Settings_GridVariants = {...
	...
	3, 'g03_Add_Cable_to_First_OH_Line'                 ,[128,100,162]/256, ':'      , 'Verst�rkung Oberleitung';...
	4, 'g04_Add_Cable_to_Weak_Cables'                   ,[  0,153,153]/256, '-'      , 'Verst�rkung Kabel';...
	};
...
Labels_Title_one_Variant     = ['Mittlerer Verlauf Spannung f�r Netzvariante "',Settings_GridVariants{Option_Active_GridVariants,2},'"'];
Labels_Title_one_Scenario    = ['Mittlerer Verlauf Spannung f�r Szenario "',Settings_Scenario{Option_Active_Scenarios,2},'"'];
	...
	for i_g = 1:size(Active_GridVariants,1)
		for i_s = 1 : size(Active_Scenarios,1)
			...
			for i_t = 1 : Data.Control.Simulation_Options.Timepoints_per_dataset
				Data_Voltage_Timeline(i_t) = nansum(nansum(squeeze(nansum(squeeze(Data.(Active_GridVariants{i_g,2}).bus_voltages(Active_Scenarios{i_s,1},:,i_t,:,:)))))) / ...
					(size(Data.(Active_GridVariants{i_g,2}).bus,1)*Data.Control.Simulation_Options.Number_of_datasets*3);
			end
			...
			if size(Active_GridVariants,1) > 1
				set(f_l, 'LineStyle', Active_GridVariants{i_g,4});
			end
			drawnow;
			if i_g == 1
		...
		Labels_Grid{end+1} = Active_GridVariants{i_g,5}; 
		f_l = plot(nan, nan);	                   % make an invisible line for legend
		set(f_l,...
			'Color', 'k',...                       % set color of invisible line
			'LineStyle', Active_GridVariants{i_g,4});    % set linestyle of invisible line
...
Labels_Title_one_Variant     = ['Histogramm f�r Netzvariante "',Settings_GridVariants{Option_Active_GridVariants,5},'"'];
Labels_Title_one_Scenario    = ['Histogramm f�r Szenario "',Settings_Scenario{Option_Active_Scenarios,5},'"'];
Labels_Title                 = 'Spannungsbandverletzungen';
Labels_X_Direction           = 'Spannungsbandverletzung in % der Profilzeit';
Labels_Y_Direction           = 'Relative H�ufigkeit [%]';
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
