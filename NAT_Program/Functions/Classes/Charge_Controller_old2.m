	...
	methods
		...
		function regulate(obj, varargin)
			...
			for i=1:numel(obj)
				...
				if any(apow_ov > 0) || obj(i).Controller_Active
% 						any((obj(i).Shifted_Energy(1:2:end) > 0) & (apow_ov < -200)) || ...
				...
				if any((obj(i).Shifted_Energy(1:2:end) > 0) & (apow_ov < -200))
					...
					pow_cha = pow_grd_l - obj(i).Values_Last_Step.Power_Vals(2,:);
