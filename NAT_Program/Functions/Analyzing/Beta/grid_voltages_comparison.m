function handles = grid_voltages_comparison(handles,observed_dataset,display_option)
% grid_voltages_comparison function compares voltages for various net variants

% grid_voltages_comparison(handles,observed_dataset,display_option);
% display_option : 'values' or 'percent' for observed dataset or 'all' for
% a simple comparison of grids, datasets for any voltage violation
...
% <no.of_grids x number of datasets x 3=(1st limit, 2nd limit, 1st+2nd]>
Comparison_of_Number_of_Violations = ...
    zeros(numel(list_of_grids),number_of_datasets,3); 
Comparison_of_Number_of_Nodes_With_Violations = ...
    zeros(numel(list_of_grids),number_of_datasets,3);
Comparison_of_Number_of_Violations_percent = ...
    zeros(numel(list_of_grids),number_of_datasets,3);
Comparison_of_Number_of_Nodes_With_Violations_percent = ...
    zeros(numel(list_of_grids),number_of_datasets,3);
...
if numel(list_of_grids) > 1 || number_of_datasets > 1
    % Is there more than one variant or more than one dataset
    ...
    end % <no.of_grids x number of datasets x 3=(1st limit, 2nd limit, 1st+2nd]>
else
    disp(' ++ Only one grid/dataset analysed - can not compare voltage violations ++ ');
    return;
end
...
barh_node_violations_grouped(bh, observed_dataset, display_option);
end

function [] = barh_node_violations_grouped(bh,observed_dataset, display_option)
    % input variables
    % structure bar horizontal with flipped values of comparison values
    if strcmp(display_option,'all') == 0
        ...
        if sum(violations(:,2)==0) == size(violations,1)
            violations(:,:,2:end) = [];
            nodes_with(:,:,2:end) = [];
            legend_text(2:end)=[];
        ...
        % Adjust position of the figure on the screen
        ...
        set(ax1,'YTick',1:1:numel(grids),'YTickLabel',grids,...
        ...
        set(ax2,'YTick',1:1:numel(grids),'YTickLabel',grids,...
            'FontName','Times New Roman','FontSize',12);
        % X label is too close to the left border, move it by 2,5 %
        ...
        if sum(floor(xtick_values - min(xtick_values)) == 0) > 1 && strcmp(display_option,'percent')==0
        ...
        % Display option 'all'. This option shows various grids at y axis.
        % Different bars represent different datasets. Only "either"
        % voltage violation is shown with this option. Currently only
        % percent are shown with this option
        
        violations = eval(['bh.Number_', 'percent']);
        nodes_with = eval(['bh.Nodes_', 'percent']);

        grids = bh.Grids;
        % If there is no additional limit, or it is never exceeded, we can
        % remove it from the graph. Therefore  there is no need for
        % the additional limit and the "any limit" value
        for i = 1 : numel(bh.Datasets)
            if i < 10
                legend_text{i}=['Dataset 0', int2str(i)];            
            else
                legend_text{i}=['Dataset ', int2str(i)];
            end
        end
        violations = violations(:,:,end);
        nodes_with = nodes_with(:,:,end);
        ...
        if numel(legend_text) == 1
            % No need to display legend, only one dataset exists
        else
            l=legend(legend_text,'Location','SouthEast');
            box(l,'off')
        end       
