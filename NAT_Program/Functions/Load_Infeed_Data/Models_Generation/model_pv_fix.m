...
% Sicherstellen, dass die Arrays gleich groß sind, durch abschneiden oder hinzufügen
% von Nullen:
if length(data_cloud_factor) ~= length(rad_dir)
	if length(rad_dir) > length(data_cloud_factor)
		rad_dir = rad_dir(1:length(data_cloud_factor));
		rad_dif = rad_dif(1:length(data_cloud_factor));
	else
		rad_dir = [rad_dir, zeros(1,length(data_cloud_factor)-length(rad_dir))];
		rad_dif = [rad_dif, zeros(1,length(data_cloud_factor)-length(rad_dif))];
	end
end
