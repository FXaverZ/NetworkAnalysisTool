...
elseif handles.Current_Settings.Load_Database.valid
	answer = questdlg({...
		...
		'   *   the load- and infeed-database, ';...
		'   *   direct from Results of a loadsimulator run (BETA), or ';...
		'   *   should the orginal input data from a previous simulation be loaded?';...
		'';...
		'Please specify desired data source:'},...
		'Specifying data source',...
...
switch answer
	case 'Direct from Loadsim (BETA)'
		mh.add_line('Source: Direct from Loadsim (BETA)');
		handles = get_input_from_loadsimulation_direct(handles);
