function Table = reformat_current_violation_for_xls_sheet(d,Grid_List)
...
for g = 1 : numel(Grid_List)
    ...
    for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
        for ds = 1 : d.Control.Simulation_Options.Number_of_datasets
            ...
            grid_table(id_,s) =  100*nansum(squeeze(d.(Grid_List{g}).current_violations(s,ds,:,:)),2)/size(d.(Grid_List{g}).branch,1);
        end
    end
    % Values
...
Table.Main_Header = 'CURRENT VIOLATIONS IN % PER TIMEPOINT/DATASET';
Table.Description = 'Current violations (table)';
