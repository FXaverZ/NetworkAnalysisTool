function Table = create_voltage_violation_time_table_per_grid(d,Data_List)
...
for i = 1 : numel(Data_List)
    ...
    for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
        for ds = 1 : d.Control.Simulation_Options.Number_of_datasets
            ...
            voltage_violations_timeline(id_,s) = nansum(squeeze(d.(Data_List{i}).voltage_violations(s,ds,:,:)),2);
        end
    end
    Table.(Data_List{i}).Values = 100*voltage_violations_timeline./size(d.(Data_List{i}).bus,1);
    Table.(Data_List{i}).Description =  ['Voltage violations at ',(Data_List{i}), ' timeline'];
    Table.(Data_List{i}).RowName =  'Number of voltage violations at grid (%)';
