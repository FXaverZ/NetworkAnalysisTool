function Table = create_branch_loading_total_time_table_per_scenario(d,Data_List)
...
for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
    ...
    Table.(['Scen_',int2str(s)]).Description =  ['Average branch loading timeline for all datasets for ',scenario_description{s}];        
    Table.(['Scen_',int2str(s)]).RowName =  'Avrg. branch dataset loading at grid (% of current limit)'; 
