function Table = create_voltage_min_total_time_table_per_grid (d,Data_List)
    ...
    for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
        for t = 1 : d.Control.Simulation_Options.Timepoints_per_dataset
			voltage_timeline(t,s) = ...
				nanmin(squeeze(nanmin(squeeze(nanmin(squeeze(d.(Data_List{i}).bus_voltages(s,:,t,:,:)))))));
        end
    end
    Table.(Data_List{i}).Values = voltage_timeline;
    Table.(Data_List{i}).Description =  ['Total Voltage minimum values for grid ',(Data_List{i}), ' timeline'];
    Table.(Data_List{i}).RowName =  'Minimum voltage level at all nodes [-]';
