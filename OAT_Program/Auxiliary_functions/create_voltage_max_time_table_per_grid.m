function Table = create_voltage_max_time_table_per_grid (d,Data_List)
    ...
    for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
        for ds = 1 : d.Control.Simulation_Options.Number_of_datasets
			...
			voltage_timeline(id_,s) = ...
				nanmax(squeeze(nanmax(squeeze(d.(Data_List{i}).bus_voltages(s,ds,:,:,:)),[],2)),[],2);
        end
    end
    Table.(Data_List{i}).Values = voltage_timeline;
    Table.(Data_List{i}).Description =  ['Voltage maximum values timelines for grid ',(Data_List{i}), ' timeline'];
