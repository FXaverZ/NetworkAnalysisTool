...
while helpdlg_condition == 0;
    user_response = questdlg(sprintf([...
        'The process will merge NAT result information files with same simulation options and grid variants.\n\n',...
        'Press Cancel in the file selection menu to end selection process!']),...
        'Merge different NAT Result files ?',...
        'Ok','Cancel','Ok');
...
user_response = questdlg([sprintf(['The following result files were selected:\n',qdlg_Text,'\n',...
    'Merge different NAT Result information files and save to ']),...
    'NVIEW\MERGED NAT', sprintf([' folder?'])],...
    'Merge different NAT Result files ?',...
    'Ok','Cancel','Ok');
...
switch user_response
    case 'Ok'
        ...
        if break_condition >= 1
            uiwait(msgbox(sprintf(['NAT results do not match and cannot be merged.\n\n',...
                'Different ', wdlg_Text,' detected!']),'Merge different NAT Result files ?'));
