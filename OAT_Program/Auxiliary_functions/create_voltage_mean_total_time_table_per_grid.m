function Table = create_voltage_mean_total_time_table_per_grid (d,Data_List)
...
for i = 1 : numel(Data_List)
    ...
    for s = 1 : d.Control.Simulation_Options.Number_of_Scenarios
        for t = 1 : d.Control.Simulation_Options.Timepoints_per_dataset
			voltage_timeline(t,s) = nansum(nansum(squeeze(nansum(squeeze(d.(Data_List{i}).bus_voltages(s,:,t,:,:)))))) / ...
				(size(d.(Data_List{i}).bus,1)*d.Control.Simulation_Options.Number_of_datasets*3);
        end
    end
    Table.(Data_List{i}).Values = voltage_timeline;
    Table.(Data_List{i}).Description =  ['Total Voltage mean values for grid ',(Data_List{i}), ' timeline'];
